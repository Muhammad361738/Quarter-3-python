...
st.title("Datasweeper streling Integrator By Muhammad Talha ")
st.write("Transfrom your files between CSV and Excel formats with built-in data cleaning ")
#  file uploader 
...
if upload_files: 
    for file in upload_files:
        ...
        if st.checkbox(f"Clean ddata for {file.name}"):
...
#                 file_name = file.name.replace(file_ext, ".csv")
#                 mime_type = "text/csv"
#             else:  # Fixed the elif to else for Excel
#                 df.to_excel(buffer, index=False)
...
#                 label=f"Download {file.name} as {conversion_type}",
#                 data=buffer,
#                 file_name=file_name,  # Fixed missing argument
#                 mime=mime_type
#             )
